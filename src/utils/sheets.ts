  ...
  if (rows.length > 1) {
    ...
    for (var i = 1; i < rows.length; i++) {
       ...
       var tea = {
         ...
         type: String(row[2] || ''),
         origin: String(row[3] || ''),
         year: isNaN(Number(row[4])) ? String(row[4] || '') : (row[4] === '' ? '' : Number(row[4])),
...
function doPost(e) {
  try {
    ...
    sheet.appendRow([
      ...
      postData.type || "",
      postData.origin || "",
      postData.year !== undefined && postData.year !== null && postData.year !== "" ? (isNaN(Number(postData.year)) ? String(postData.year) : Number(postData.year)) : "",
      postData.producer || "",
